...
def save_wifi(ssid, password):
	...
	with open("modeconfig.json", "r") as fo:
		cfg = fo.read()
	with open("modeconfig.json", "w+") as fo:
		cfg = json.loads(cfg)
		cfg['wifi'] = wifidata
		change = False
		for i in cfg['wifi_history']:
			if i[0] == ssid:
				i[1] = password
				change = True
				break
		if len(cfg['wifi_history']) < 4 and not change:
			cfg['wifi_history'].append([ssid, password])
		elif not change:
			cfg['wifi_history'].pop(0)
			cfg['wifi_history'].append([ssid, password])
...
def _httpHanderConfig(httpClient, httpResponse) :
	...
	if other_flg == 'true' or do_connect(ssid, password):
		...
		content = """<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><meta http-equiv="X-UA-Compatible" content="ie=edge"><style>h1{text-align:center;margin-top:120px;font-size:1.4rem;color:#0064a0}p{text-align:center;font-size:1.1rem}div{width:200px;margin:0 auto;padding-left:60px}</style><title></title></head><body><h1>^_^ WiFi connection success</h1><div><span>Resetting device now </span><span id="waiting">...</span></div></body><script>var waiting=document.getElementById("waiting");setInterval(function(){if(waiting.innerText==="..."){return waiting.innerText="."}waiting.innerText+="."},500);</script></html>"""
	else:
